    ...
    public static void CalculateBoundingBoxes(ModelContent output, TargetPlatform platform)
    {
      ...
      foreach (var mesh in output.Meshes)
      {
        ...
        foreach (var part in mesh.MeshParts)
        {
          unsafe
          {
            fixed (byte* b = mesh.VertexBuffer.VertexData)
            {
              int offset = 0;
              int stride = 0;
              foreach (VertexElement ve in part.GetVertexDeclaration())
              {
                if (ve.UsageIndex == 0 && ve.VertexElementUsage == VertexElementUsage.Position)
                {
                  ...
                  {
                    throw new InvalidContentException(
                        String.Format("Can't deal with vertex stream {0} for Position0!",
                            ve.Stream));
                  }
                  offset = ve.Offset;
                }
                int s = KiloWatt.Base.Graphics.VertexDeclarationContent.FormatSize(ve.VertexElementFormat)
                    + ve.Offset;
                if (s > stride)
                  stride = s;
              }
              offset += part.StreamOffset;
              offset += part.BaseVertex * stride;
